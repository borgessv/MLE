...
Description: class prediction using a linear SVM algorithm.
...
from utils import create_dataset, normalization, denormalization
...
train_split = 0.5                                                              # options: any value in [0, 1]    
shuffle = True                                                                  # options: True or False
margin = "hard"                                                                 # options: "hard" or "soft"
seed = False                                                                    # options: False or any integer number
optimizer = "quadprog"                                                      # options: "cvxopt" or "quadprog"

plot_filename = dataset + "_" + str(train_split)                                # filename used in plots  
...
match dataset:                                                                  # loads or creates dataset
    case "random":
        data = create_dataset(n=1000, cov_factor=0.1)                                    
    case other:
...
lm, sp = model.solve_qp(solver=optimizer)
# match optimizer:
#     case "cvxopt":
#         lm, sp = model.cvxopt_solve_qp()                                        # solves the convex quadratic optimization problem using 'cvxopt' library   
#     case "quadprog":
#         lm, sp = model.quadprog_solve_qp()                                      # solves the convex quadratic optimization problem using 'quadprog' library   
#     case other:
#         raise Exception("Optimization algorithm not found!")
...
if model.normalize is True:                                                     # normalizes the data to be predicted (if applicable)
...
x_test_fc1 = x_test[false_c1] 
x_test_fc2 = x_test[false_c2] 
...
if model.normalize is True:                                                     # denormalizes points to be plotted (if applicable)
    ...
    c1_data_train = denormalization(model.raw_data,                             # class 1 - train examples 
                            model.train_data[model.train_data[:, 2] == 1, :-1])                             
    c2_data_train = denormalization(model.raw_data,                             # class 2 - train examples
                            model.train_data[model.train_data[:, 2] == -1, :-1])         
    c1_data_test = denormalization(model.raw_data,                              # class 1 - test examples 
                            x_test[y_test_true == 1])                             
    c2_data_test = denormalization(model.raw_data,                              # class 2 - test examples
                            x_test[y_test_true == -1])  
    x_test_fc1 = denormalization(model.raw_data, x_test_fc1)                    # class 2 - misclassified examples
    x_test_fc2 = denormalization(model.raw_data, x_test_fc2)                    # class 1 - misclassified examples
...
if x_test_fc2.size != 0:
    plt.plot(x_test_fc2[:,0], x_test_fc2[:,1],'ob', label="Al - false")
if x_test_fc1.size != 0:
    plt.plot(x_test_fc1[:,0], x_test_fc1[:,1], 'or', label="Cu - false") 
...
#%% Confusion matrix
confusion_matrix = metrics.confusion_matrix(y_test_true, y_test)
cm_display = metrics.ConfusionMatrixDisplay(confusion_matrix = confusion_matrix, display_labels = ["Al", "Cu"])
cm_display.plot()
